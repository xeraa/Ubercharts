 ...
 * Numbers, Label/Y-Pairs, X/Y-Coordinates and Points to be plotted on the
 * chart.
 * <p/>
...
public class MixedSeries extends AbstractSeries<Object[]> {
	...
	/**
	 * Sets a two-dimensional array of Label/Y-Pairs to be plotted on the chart.
	 * The corresponding X-Values will be automatically calculated by
	 * Highcharts, counting from 0 on.
	 * 
	 * The second dimension of the array must have the length of 2, to represent
	 * Label/Y pairs. Additionally the first field of the second dimension must
	 * be a String value representing the label. The second field must be an
	 * instance of {@link Number}. If that is not the case, an
	 * {@link java.security.InvalidParameterException} will be thrown.
	 * 
	 * @param pData
	 *            a two-dimensional array of Label/Y-Pairs
	 * 
	 * @return this object for convenient chaining, not a copy
	...
	/**
	 * Sets an collection with data points of different types to be drawn as a
	 * series on the chart.
	 * <p/>
	 * Only the following types are allowed to be in the collection, otherwise
	 * an {@link IllegalArgumentException} will be thrown.
	 * <p/>
	 * - {@link Point}s - {@link Number}s - Array with length of 2 and
	 * containing a Number/Number pair - Array with length of 2 and containing a
	 * String/Number pair
	 * 
	 * @param pMixedData
	 *            an collection with data points of different types to be drawn
	 *            as a series on the chart
	 * @return this object for convenient chaining, not a copy
	 */
		...
		for (Object data : pMixedData) {
			...
			if (!(isNumberOrPoint(data) || isValidArrayContainingAPoint(data))) {
				throw new IllegalArgumentException(
						"The mixed data series can only contain Numbers, Points, and Arrays which "
	...
	/**
	 * Helper method which checks the given object for its type. If it's
	 * {@link Number} or {@link Point}, {@code true} will be returned, otherwise
	 * {@code false}.
	 * 
	 * @param pObject
	 *            the object to be checked
	 * @return {@code true} it the given object's type is {@link Number} or
	 *         {@link Point}, otherwise {@code false}.
	 */
	...
	/**
	 * Helper method which checks the given object for being an array containing
	 * a valid series point which can be used within a mixed series.
	 * <p/>
	 * It is valid if it is an array with the length of 2 and containing a
	 * Number/Number-Pair or a String/Number-Pair.
	 * <p/>
