 ...
 * Label/Y-Pairs to be plotted on the chart. The corresponding X-Values will be
 * automatically calculated by Highcharts, counting from 0 on.
 * <p/>
 * For example: If you set the Y-Values to new Integer[]{2,6,9}, Highcharts will
 * plot the points (0/2), (1/6) and (2/9).
 * <p/>
...
public class LabeledNumberSeries extends AbstractSeries<Object[][]> {
		...
		for (int i = 0; i < pData.length; i++) {
			if (!(pData[i][0] instanceof String && pData[i][1] instanceof Number)) {
				throw new InvalidParameterException(
						"Invalid DataSet: All entries must consist of String-Number pairs.");
			}
		}
	...
	/**
	 * Sets two arrays to represent labels and Y values for coordinates to be
	 * plotted on the chart. Both array must have the same length or an
	 * {@link IllegalArgumentException} will be thrown. Every y-Value will be
	 * paired with a label, stored at the same array index. The corresponding
	 * X-Values will be automatically calculated by Highcharts, counting from 0
	 * on.
	 * 
	 * @param pLabels
	 *            an array representing the labels
	 * @param pYValues
	...
	public LabeledNumberSeries setData(String[] pLabels, Number[] pYValues) {
		...
		if (pLabels.length == pYValues.length) {
			...
			throw new IllegalArgumentException(
					"The arrays for labels and Y values must have the same length, but they have "
	...
	/**
	 * Sets two arrays to represent Labels and Y values for coordinates to be
	 * plotted on the chart. Both arrays must have the same length or an
	 * {@link IllegalArgumentException} will be thrown. Every X-Value will be
	 * paired with a label, stored at the same array index. The corresponding
	 * X-Values will be automatically calculated by Highcharts, counting from 0
	 * on.
	 * 
	 * @param pLabeledNumbers
	 *            a map representing label/y-pairs
	 * @return this object for convenient chaining, not a copy
	 */
